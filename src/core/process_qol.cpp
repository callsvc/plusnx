...
namespace Plusnx::Core {
    ProcessQol::ProcessQol(const SysFs::SysPath &database) {
        std::print("Loaded SQLite library version: {}\n", sqlite3_libversion());
    ...
    void ProcessQol::ChangeGame(const GameInformation &game) {
        ...
        const auto currTime{std::format("{:%D}", chr::floor<chr::seconds>(chr::system_clock::now()))};
        const auto session{std::format("{} started the game named {} at {}", GetUserName(), game.title, currTime)};
        const auto command{std::format("INSERT INTO Played(SessionText) VALUES('{}');)", session)};
    ...
    std::vector<std::string> ProcessQol::GetPlayedSessions() {
